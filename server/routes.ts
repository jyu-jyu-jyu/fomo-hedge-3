...
export function registerRoutes(httpServer: Server, app: Express) {
  ...
  app.patch("/api/me", (req, res) => {
    try {
      ...
      // Format: name@mba2027.hbs.edu
      const match = hbsEmail?.match(/^[^@]+@mba(\d{4})\.hbs\.edu$/i);
      if (!match) {
        return res.status(400).json({ error: "Must be a valid HBS email (e.g. jsmith@mba2027.hbs.edu)" });
      }
      // Extract 4-digit class year from domain e.g. mba2027.hbs.edu → 2027
  ...
  app.post("/api/sync/:provider", async (req, res) => {
    ...
    try {
      ...
      if (provider === "eventbrite") {
        const r = await fetch(
          "https://www.eventbriteapi.com/v3/users/me/orders/?expand=event",
